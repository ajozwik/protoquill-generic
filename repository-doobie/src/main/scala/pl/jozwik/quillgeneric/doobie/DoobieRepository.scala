...
trait DoobieRepositoryWithTransactionWithGeneratedId[K, T <: WithId[K], C <: DoobieJdbcContextWithDateQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
  extends RepositoryMonadWithTransactionWithGeneratedId[ConnectionIO, K, T, C, D, N, Long]
  with DoobieJdbcTransaction[K, T, C, D, N]
trait DoobieRepository[K, T <: WithId[K], C <: DoobieJdbcContextWithDateQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
  extends RepositoryMonadWithTransaction[ConnectionIO, K, T, C, D, N, Long]
  with DoobieJdbcTransaction[K, T, C, D, N]

trait DoobieJdbcTransaction[K, T <: WithId[K], C <: DoobieJdbcContextWithDateQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
