...
import pl.jozwik.quillgeneric.zio.ZioJdbcRepository.{ QIO, ZioJdbcContextWithDataQuotes }
import zio.ZIO
...
object ZioJdbcRepository {
  ...
  type ZioJdbcContextWithDataQuotes[+Dialect <: SqlIdiom, +Naming <: NamingStrategy] = ZioJdbcContext[Dialect, Naming]
    with ObjectGenericTimeDecoders
    with ObjectGenericTimeEncoders
}

trait ZioJdbcRepositoryWithTransactionWithGeneratedId[K, T <: WithId[K], C <: ZioJdbcContextWithDataQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
  extends JdbcRepositoryMonadWithGeneratedId[QIO, K, T, C, D, N, Long]
  with ZioJdbcRepositoryBase[K, T, C, D, N]
trait ZioJdbcRepository[K, T <: WithId[K], C <: ZioJdbcContextWithDataQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
  extends JdbcRepositoryMonad[QIO, K, T, C, D, N, Long]
  with ZioJdbcRepositoryBase[K, T, C, D, N]

trait ZioJdbcRepositoryBase[K, T <: WithId[K], C <: ZioJdbcContextWithDataQuotes[D, N], +D <: SqlIdiom, +N <: NamingStrategy]
